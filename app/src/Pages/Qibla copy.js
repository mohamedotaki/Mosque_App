...
export default function Qibla() {
  ...
  function calculate(lat, lon) {
    ...
    var latk = (21.4225 * PI) / 180.0;
    var longk = (39.8264 * PI) / 180.0;
    var phi = (lat * PI) / 180.0;
    var lambda = (lon * PI) / 180.0;
    var qiblad =
      (180.0 / PI) *
      Math.atan2(
  ...
  if (position.latitude !== null && position.longitude !== null) {
    calculate(position.latitude, position.longitude);
  }
  useEffect(() => {
      ...
      navigator.geolocation.getCurrentPosition(
        function (position) {
          setPosition({
            ...
            userFacing: position.coords.heading,
          });
        },
        (e) => alert(e.message)
      );
  ...
  function handleOrientation(event) {
    setX(event.beta); // In degree in the range [-180,180)
    setY(event.gamma); // In degree in the range [-90,90)
  }
    ...
    <div>
      <div className="qiblaContainer">
        <svg
          ...
          transform="matrix(-1, 0, 0, 1, 0, 0)rotate(111)"
        >
      ...
      {position.latitude && position.longitude ? (
        <p>
          ...
          userFacing: {position.userFacing}
        </p>
      ) : (
        <p>Loading...</p>
      )}
